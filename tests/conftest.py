"""
pytest configuration for the SparseLab test suite.
...
import mode can add the workspace root (/SparseLab/) to sys.path[0],
which makes it prefer the raw folder `sparselab/` over the properly
...
installed sparselab package (via pip install -e .) is then resolved
